# MyParking

Database used: Firebase

Work Distribution (functionality wise):

Rajdeep Dodiya - 101320088

(Front-End):
 - Sign In Screen with email validation
 - Sign Up Screen with email and carplate number validations
 - Parking List screen using tableView and Delete Parking functionality
 - Display Detailed Parking with MapView functionality
 - Update Profile screen
 - Implementation of UserDefaults for Remember Me functionality
 - Logout functionality in Navigation Bar
 - Delete User function in Firebase DBHelper file
 
 Farithul Inzamam - 101340711 
 
 (Back-End):
 - All Firebase DBHelper functions with appropriate error-handling.
 - Internet connection check performed at every step possible. 
 - Splash Screen and Tab Bar view of ParkingList and User Profile screens.
 - Implemented functionality where a user can add more than one car and it's relevant parkings.
 - Add New Parking screen with building code, suite number and address validations.
 - Getting user location
 - User Profile screen with Add New Car functionality

----------------------------------------------------------------------------------------------------------------------------------------
# Screenshots

Firebase:

<img src="Screenshots/Screenshot%202021-05-18%20at%208.25.24%20PM.jpg" >

Sign In Page:

<img src="Screenshots/Sign_In_Screen" width="276" height="599" >

Sign up Page:

<img src="Screenshots/Sign_Up_Screen" width="276" height="599" >

Sign Up Page validations:
<img src="Screenshots/Sign_Up_Screen2" width="276" height="599" > <img src="Screenshots/Sign_Up_Screen3" width="276" height="599" > <img src="Screenshots/Sign_Up_Screen4" width="276" height="599" > 

Add Parking Page:

<img src="Screenshots/Add_Parking_Screen_Before" width="276" height="599" >

Add Parking Page Validations:
<img src="Screenshots/Add_Parking_Screen_Car_Selection" width="276" height="599" > <img src="Screenshots/Add_Parking_Screen_No_Of_Hours_Selection" width="276" height="599" > <img src="Screenshots/Add_Parking_Screen_After" width="276" height="599" >

Parking List Page:

<img src="Screenshots/Parking_List_Screen" width="276" height="599" >

Parking Detail Page:

<img src="Screenshots/Parking_Detail_Screen" width="276" height="599" >

Show Directions Page:

<img src="Screenshots/Show_Directions_Screen" width="276" height="599" >

User Profile Page:

<img src="Screenshots/User_Profile_Screen" width="276" height="599" >

Add Car Functionality:

<img src="Screenshots/Add_Car_Functionality" width="276" height="599" >

Update Profile Page:

<img src="Screenshots/Update_Profile_Screen" width="276" height="599" >

User Profile Page after Update adn Add New Car functionality:

<img src="Screenshots/User_Profile_Screen_After_Update" width="276" height="599" >

